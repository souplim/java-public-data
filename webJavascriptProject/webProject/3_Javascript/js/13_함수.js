...
document.getElementById("btn2-3").addEventListener("click", () => {
    ...
    // 함수 호출(화살표 함수)
    printConsole(num => num * 5 ); // {}, return 동시에 제거해야만?

    // 반환값이 Object이면 {}, return 생략 불가
    // -> 화살표(=>) 다음에는 함수 정의 부분이 있어야 되는데 
    //    객체(Object)가 작성되어 있어 문법 오류가 발생

    // printConsole(num => { price : num*1000, n : num }); // 문법 오류
...
// this(이벤트 발생한 요소) 사용 불가

// 1) 익명 함수는 this( <button id="btn2-4">this 사용 불가</button> ) 사용 가능
...
document.getElementById("btn2-4").addEventListener("click", (e) => {
    ...
    console.log(this); 
    // this.style.color = "gold"; // 실행 안 됨

    // this와 같은 e.target 이용해서 글자색 변경
    e.target.style.color = "gold"; // 글자색 변경
})