...
$(document).ready(function(){
    ...
    // -> 배열에서 요소를 하나씩 꺼내서 적용하는 건 가능하나 불편함
...
$(document).ready(function(){
    ...
    // 클래스를 item, select를 동시에 가지고 있는 요소만 글자 크기 10px로 변경
...
$("#input1").on("input", function(){
    ...
    if(regExp.test( $("#input1").val() ) ){
        ...
        // "유효하지 않은 문자열 형식 입니다" , 글씨 red
        $("#span1").text("유효하지 않은 문자열 형식 입니다").css("color","red");

        // method chaining : 하나의 대상에 대하여 여러 메서드를 연달아 작성하는 기술
    }
});
...
$(function(){
    ...
    // 클래스가 area인 요소의 후손 중 ul 후손 중에 클래스가 qqq인 요소의 배경색 바꾸기
    $(".area > ul > .qqq").css("backgroundColor","gold");

    // 클래스 area의 후손 중 클래스가 qqq인(오렌지, 멜론, 테스트2) 요소의 폰트 크기를 30px로 변경
    $(".area .qqq").css("fontSize","30px");

    // 내용이 "사과"인 요소를 선택해서 배경색 red 글씨 white로 변경
...
$("#check").on("click", function(){
    ...
    console.log(gender.length); // 하나 check 되어 있으면 1 아니면 0

    // 변수 gender는 Javascript 방식의 변수
    // -> 이후 gender를 단순하게 호출하게 되면 JS 방식으로 사용해야 함
    ...
    if(gender.length == 0){
        ...
        // 2) 체크된 요소를 모두 얻어와도
        //    radio는 1개만 체크되어 있기 때문에 변수 한 개랑 같다라고 해석하여
        //    자동으로 0번 인덱스 요소에 있는 value를 얻어옴 (JS + jQuery)
        console.log(gender.val());

        // 3) 순수 jQuery
        console.log($(gender).val());
        // $(gender) : 체크된 요소만 담긴 배열 + 요소 선택해라 기호
...
$("#btn1").on("click", function(){
    ...
    // 배열명을 적을 경우 0번 인덱스만 확인 가능 -> 축구 눌렀을 때만 true 나옴
    ...
    for(let i=0; i<arr.length; i++){
        ...
        if($(arr[i]).prop("checked")){
            // 체크된 요소의 value값 얻어와 str변수에 누적
            str += $(arr[i]).val()+ " ";  // 에러 const -> 상수(변할 수 없는 수)
        }
    }
